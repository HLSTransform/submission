...
struct RunState
{
  ...
  QuantizedTensor<dim> xq;               // quantized x (dim,)
  QuantizedTensor<hidden_dim> hq;        // quantized hb (hidden_dim,)
  float q[dim];                          // query (dim,)
  float k[(dim * n_kv_heads) / n_heads]; // key (dim,)
  float v[(dim * n_kv_heads) / n_heads]; // value (dim,)
  float att[n_heads * seq_len];          // buffer for scores/attention values (n_heads, seq_len)
  // float logits[vocab_size];              // output logits
  //  kv cache
  float key_cache[n_layers * seq_len * ((dim * n_kv_heads) / n_heads)];   // (layer, seq_len, dim)
  float value_cache[n_layers * seq_len * ((dim * n_kv_heads) / n_heads)]; // (layer, seq_len, dim)
};
...
struct Transformer
{
  ...
  RunState<dim, hidden_dim, n_layers, n_heads, n_kv_heads, vocab_size, seq_len, GS> state;             // buffers for the "wave" of activations in the forward pass
